...
class TriangulatedAnalyticsEngine:
    ...
    def _analyze_pass_data(self, pass_assessment) -> Dict:
        ...
        if not pass_assessment or not hasattr(pass_assessment, 'factors') or not pass_assessment.factors:
    ...
    def _analyze_cat4_data(self, cat4_assessment) -> Dict:
        ...
        if not cat4_assessment or not hasattr(cat4_assessment, 'domains') or not cat4_assessment.domains:
            # Return empty structure with all expected fields
            return {
                'available': False,
                'domains': [],
        ...
        for domain in cat4_assessment.domains:
            # Use actual SAS score if available, otherwise convert from stanine
            if hasattr(domain, 'sas_score') and domain.sas_score:
                sas = domain.sas_score
            else:
    ...
    def _analyze_academic_data(self, academic_assessment) -> Dict:
        """Academic Subject Analysis as per instruction set - CORRECTED FOR YOUR DATABASE"""
        if not academic_assessment or not hasattr(academic_assessment, 'subjects') or not academic_assessment.subjects:
            # Return empty structure with all expected fields
            return {
                'available': False,
                'subjects': [],
        ...
        for subject in academic_assessment.subjects:
            # Use 'stanine' - this is your actual database column name
            stanine = subject.stanine  # NOT subject.internal_stanine
            ...
            subjects.append({
                'name': subject.name,
                'stanine': stanine,
                'marks': getattr(subject, 'marks', None),  # May not exist in your schema
                'level': level,
        ...
        return {
            ...
            'weaknessAreas': weakness_areas,
            'strengthAreas': strength_areas,
            'averageStanine': np.mean([s['stanine'] for s in subjects]) if subjects else 0
        }
    ...
    def _generate_interventions(self, pass_analysis: Dict, cat4_analysis: Dict, academic_analysis: Dict, is_fragile_learner: bool) -> List[Dict]:
        """Intervention Strategy Mapping as per instruction set - FIXED FOR YOUR API MODELS"""
        interventions = []
            ...
            for risk in pass_analysis.get('riskAreas', []):
                ...
                if p_number in ['P3', 'P7']:
                    interventions.append({
                        ...
                        'domain': 'emotional',
                        'factor': factor_name,  # Added missing factor field
                        'title': 'Self-esteem/confidence building',  # Added missing title field
                    ...
                    interventions.append({
                        ...
                        'domain': 'behavioral',
                        'factor': factor_name,  # Added missing factor field
                        'title': 'Time management / Organization skills',  # Added missing title field
                    ...
                    interventions.append({
                        ...
                        'domain': 'behavioral',
                        'factor': factor_name,  # Added missing factor field
                        'title': 'Attendance and engagement mentoring',  # Added missing title field
            ...
            for weakness in cat4_analysis.get('weaknessAreas', []):
                if weakness['domain'] == 'Verbal' and weakness['sas'] < 90:
                    interventions.append({
                        ...
                        'domain': 'cognitive',
                        'factor': f"CAT4 {weakness['domain']}",  # Added missing factor field
                        'title': 'Verbal reasoning / reading boosters',  # Added missing title field
        ...
        if is_fragile_learner:
            interventions.append({
                ...
                'domain': 'holistic',
                'factor': 'Multiple CAT4 domains',  # Added missing factor field
                'title': 'Holistic learning support',  # Added missing title field
        ...
        if academic_analysis.get('available'):
            for weakness in academic_analysis.get('weaknessAreas', []):
                interventions.append({
                    ...
                    'domain': 'academic',
                    'factor': weakness["subject"],  # Added missing factor field
                    'title': f'Subject-specific booster modules - {weakness["subject"]}',  # Added missing title field
